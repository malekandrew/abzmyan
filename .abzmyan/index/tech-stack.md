...
## Key dependencies
- **`prompts`** (`^2.4.2`) — interactive CLI prompts used in `src/commands/init.js` for project code / mode / AI agent selection questions, and in `src/commands/update.js` for the add/change-agents prompt.
...
- `npx abzmyan update` — refresh `.claude/commands/*` agent templates in place; also migrates retired ticket statuses and the old deploy config shape, and removes retired agents' command files.
...
- The agent prompt files (`templates/commands/*.md`) are the actual behavioral spec for the 4-agent per-ticket workflow, plus Bootstrapper and the ticket-agnostic Deployer; changes to workflow behavior are made there, not in JS source.
